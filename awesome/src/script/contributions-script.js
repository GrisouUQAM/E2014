...
tabSelected = "Articles", userCount;
...
function getJsonWiki() {
  ...
  if(tabSelected === "Articles")
  {
    ...
    getUserContributionQuery(wiki, user);
...
function doGetCount(url)
{
		...
		type: 'GET',
		success: function (response){
			userCount = response.userdailycontribs.totalEdits;
			$("#TotalCount").text("    User has made " +userCount + " total contributions.");
      }
	  });
}
 ...
 function getUserContributionQuery(wiki, user)
{
       var query = wiki + "/w/api.php?action=userdailycontribs&format=json&user=" + user + "&daysago=60000";
       doGetCount(query);
}
